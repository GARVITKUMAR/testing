 ...
 * 
 * @version 5.00	2019-02-26 The implementation of UI for Programmable Calculator
 */
...
public class UserInterface {
	...
	private Label label_DoubleCalculator = new Label("Science & Engineering Calculator with Units");
	...
	private Button Progbtn = new Button("Program");
	private Button newbtn = new Button("New Button");
	...
	private Label lbl_saveResult = new Label("Save Result");
	private CheckBox saveResult = new CheckBox();
...
NewInterface resultUI1;
	...
	public UserInterface(Pane theRoot) {
		...
		setupLabelUI(lbl_saveResult, "Arial", 15, Calculator.WINDOW_WIDTH, Pos.BASELINE_LEFT, 10, 280);
		saveResult.setLayoutX(35);
		saveResult.setLayoutY(300);
		...
		setupButtonUI(newbtn, "Symbol", 22, BUTTON_WIDTH, Pos.BASELINE_LEFT, 1 * buttonSpace - BUTTON_OFFSET, 420);
		...
		setupButtonUI(Progbtn, "Symbol", 22, BUTTON_WIDTH, Pos.BASELINE_LEFT, 5 * buttonSpace - BUTTON_OFFSET, 420);
		Progbtn.setOnAction((event) -> {try {
			Result();
		} catch (IOException e) {
		...
		theRoot.getChildren().addAll(label_DoubleCalculator, label_Operand1, text_Operand1, label_errOperand1,
				...
				cmbox_operand2_unit, cmbox_result_unit, saveResult, lbl_saveResult,Progbtn, newbtn);
	...
	private void Result() throws IOException {
		...
		resultStage.setTitle("Program");
resultUI = new ProgramUI(resultStage);

	}
	
	private void Result1() throws IOException {
		Stage resultStage1 = new Stage();
		resultStage1.setTitle("Program");
resultUI1 = new NewInterface(resultStage1);
		...
		if (numeric) {
			...
			text_Operand1.setOnAction((event) -> {
				try {
					...
					setOperand1:
					if (a.get_table() != true) {						ManageDefinitions h = new ManageDefinitions();
	...
	private void save_result() throws IOException {
		DefinitionsUserInterface a = new DefinitionsUserInterface();
		...
		try (FileWriter writer = new FileWriter(name)) {
			writer.write("\n");
			writer.write(result + " " + result_err + " " + result_unit + "\n");
		} catch (IOException e) {
			e.printStackTrace();
		}
		return;
